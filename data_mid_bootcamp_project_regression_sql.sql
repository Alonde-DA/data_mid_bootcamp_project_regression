...
select distinct bedrooms, round(avg(price),0) as avg_price from house_price_data
group by bedrooms
...
select distinct bedrooms, round(avg(sqft_living),0) as avg_sqft_living from house_price_data
group by bedrooms
order by bedrooms asc;
...
-- there is no clear correlation for grades as they all (except grade 3) have an average condition around 3. For the houses with a condition above 3, avg grade is above 7. 
